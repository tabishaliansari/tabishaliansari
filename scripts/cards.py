...
These are files in your own repo, so they render as long as GitHub renders.
    ...
    python scripts/cards.py --user tabishaliansari --out assets
...
A projects.json entry may name a repo as "owner/repo" to card a repo that
lives in an org rather than on --user, and may set "name" to override the
title and filename (useful when the real repo name is very long).
...
THEMES = {
    "dark": {
        "bg": "#0d1117", "border": "#262c34", "title": "#ededed",
        "text": "#b1bac4", "muted": "#7d8590", "value": "#ffffff",
        "accent": "#ededed",
    },
    "light": {
        "bg": "#ffffff", "border": "#e5e5e5", "title": "#0a0a0a",
        "text": "#3f3f46", "muted": "#737373", "value": "#0a0a0a",
        "accent": "#0a0a0a",
    },
}
...
def main(argv=None):
    ...
    for entry in wanted:
        spec = entry["repo"]
        if "/" in spec:
            # owner/repo - a repo in an org, or anywhere outside this account
            try:
                src = rest("/repos/" + spec, token)
            except Exception as exc:
                print(f"  !! {spec} not reachable ({exc}), skipped")
                continue
        else:
            src = by_name.get(spec.lower())
        if not src:
            print(f"  !! {spec} not found on the account, skipped")
            continue
        label = entry.get("name") or src["name"]
        card = {
            "name": label,
        ...
        for theme in ("dark", "light"):
            dest = args.out / f"card-{label}-{theme}.svg"
            dest.write_text(render_repo(card, theme), encoding="utf-8")
        print(f"wrote card-{label}-*.svg  "
